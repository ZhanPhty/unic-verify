 ...
 * 构造统一的Input格式
 */
import type { PubicEvent, InputOnlyHasCurrent, Input, Point, SupportElement } from '@unic/shared'
import { CLIENT_X, CLIENT_Y, TYPE_START, TYPE_CANCEL, TYPE_END } from '@unic/shared'
...
function getCenter(points: { clientX: number; clientY: number }[]): Point | void {
  ...
  if (0 < length) {
    ...
    const countPoint = points.reduce(
      (countPoint: Point, point: { clientX: number; clientY: number }) => {
...
function extendInput(
  basicsInput: PubicEvent,
  id: number
): Omit<Input, 'prevInput' | 'startInput' | 'startMultiInput'> {
